class Foto {
      ...
      (a.image = new Image()),
        (a.image.onload = function () {
          (a.imageWidth = a.image.width),
            ...
            (a.imageData = []),
            a.operationOrgCtx.clearRect(
              0,
              0,
              a.operationOrgCanvas.width,
              a.operationOrgCanvas.height
            ),
            a.operationEditedCtx.clearRect(
              0,
              0,
              a.operationEditedCanvas.width,
              a.operationEditedCanvas.height
            ),
            a.operationOrgCtx.drawImage(a.image, 0, 0),
            a.operationEditedCtx.drawImage(a.image, 0, 0),
            a.previewImage(a.operationOrgCanvas, 0),
            (a.imageData = a.operationOrgCtx.getImageData(
              0,
              0,
              a.operationOrgCanvas.width,
              a.operationOrgCanvas.height
            )),
            a.generatePixelMatrix(),
  ...
  generatePixelMatrix() {
    ...
    (this.redPixelMatrix = []),
      ...
      (this.bluePixelMatrix = []),
      (this.alphaPixelMatrix = []);
    for (var r = 0; r < this.imageData.data.length; r += 4)
      (r / 4) % this.imageWidth == 0 &&
        (0 != r &&
          (this.redPixelMatrix.push(t),
        ...
        (a = []),
        (i = [])),
        t.push(this.imageData.data[r]),
        e.push(this.imageData.data[r + 1]),
        a.push(this.imageData.data[r + 2]),
        i.push(this.imageData.data[r + 3]);
  }
  grayscale() {
    ...
    this.operationEditedCtx.putImageData(t, 0, 0),
      this.operationOrgCtx.putImageData(t, 0, 0),
      this.previewImage(),
  ...
  makeTransparent() {
    for (var t = this.imageData, e = 0; e < t.data.length; e += 4)
      Math.abs(t.data[e] - this.pickedR) < 30 &&
        Math.abs(t.data[e + 1] - this.pickedG) < 30 &&
        Math.abs(t.data[e + 2] - this.pickedB) < 30 &&
        (t.data[e + 3] = 0);
    this.operationEditedCtx.putImageData(t, 0, 0), this.previewImage();
  }
  applyFilter(t) {
    for (var e = 0; e < this.imageData.data.length; e += 4) {
      ...
      if (
        0 != a &&
        0 != i &&
        a != this.imageHeight - 1 &&
        i != this.imageWidth - 1
      ) {
        for (var r = 0, s = 0, h = 0, o = 0, d = 0; d < 3; d++)
          for (var n = 0; n < 3; n++)
            null != this.redPixelMatrix[a + (d - 1)] &&
              null != this.redPixelMatrix[a + (d - 1)][i + (n - 1)] &&
              ((r += t[d][n] * this.redPixelMatrix[a + (d - 1)][i + (n - 1)]),
              (s += t[d][n] * this.greenPixelMatrix[a + (d - 1)][i + (n - 1)]),
              (h += t[d][n] * this.bluePixelMatrix[a + (d - 1)][i + (n - 1)]),
              (o += t[d][n] * this.alphaPixelMatrix[a + (d - 1)][i + (n - 1)]));
        this.convertedToGrayScale
          ? ((this.imageData.data[e] = (r + s + h) / 3),
            (this.imageData.data[e + 1] = (r + s + h) / 3),
            (this.imageData.data[e + 2] = (r + s + h) / 3),
            (this.imageData.data[e + 3] = o))
          : ((this.imageData.data[e] = r),
            (this.imageData.data[e + 1] = s),
  ...
  flipVertically() {
    this.operationEditedCtx.translate(this.imageWidth, 0),
      ...
      (this.imageData = this.operationOrgCtx.getImageData(
        0,
        0,
        this.operationOrgCanvas.width,
        this.operationOrgCanvas.height
      )),
  ...
  rotate(t) {
    this.operationEditedCtx.clearRect(
      0,
      0,
      this.operationEditedCanvas.width,
      this.operationEditedCanvas.height
    ),
      this.operationEditedCtx.save(),
      this.operationEditedCtx.translate(
        this.imageWidth / 2,
        this.imageHeight / 2
      ),
      this.operationEditedCtx.rotate((t * Math.PI) / 180),
      this.operationEditedCtx.drawImage(
        this.image,
        -this.image.width / 2,
        -this.image.width / 2
      ),
      this.operationEditedCtx.restore(),
      this.operationOrgCtx.clearRect(
        0,
        0,
        this.operationOrgCanvas.width,
        this.operationOrgCanvas.height
      ...
      this.operationOrgCtx.drawImage(
        this.image,
        -this.image.width / 2,
        -this.image.width / 2
      ),
  ...
  export() {
    ...
    (t.download = this.selectedFileName + "-LightningFilter.png"),
  ...
  previewImage(t, e, a) {
    ...
    null != e &&
      0 == e &&
      (this.previewImageElement.addEventListener("mouseover", function (t) {
        this.style.cursor = "crosshair";
      }),
      this.previewImageElement.addEventListener("click", function (t) {
        (i.relativeStartX = t.offsetX),
          (i.relativeStartY = t.offsetY),
          i.ctrlPressed && i.pickColorPixel(i.relativeStartX, i.relativeStartY),
          (i.selectStart = !1);
      }),
      ...
      this.previewImageElement.addEventListener("mousemove", function (t) {
        (i.endX = t.clientX),
          (i.endY = t.clientY),
          i.selectStart &&
            ((i.selectRect.style.position = "fixed"),
            (i.selectRect.style.display = "initial"),
            (i.selectRect.style.border = "2px dashed black"),
            (i.selectRect.style.top = i.startY + "px"),
            (i.selectRect.style.left = i.startX + "px"),
            (i.selectRect.style.height = i.endY - i.startY + "px"),
            (i.selectRect.style.width = i.endX - i.startX + "px"));
      }),
      this.previewImageElement.addEventListener("mouseup", function (t) {
        (i.relativeEndX = t.layerX),
          (i.relativeEndY = t.layerY),
          (i.selectStart = !1),
          (i.selectRect.style.height = "0px"),
          (i.selectRect.style.width = "0px"),
          (i.selectRect.style.display = "none");
      }),
      this.selectRect.addEventListener("mouseup", function (t) {
        i.selectStart = !1;
      })),
      (this.previewImageElement.src =
        null == t ? i.operationEditedCanvas.toDataURL() : t.toDataURL());
  }
  recreateImageObject() {
  ...
  applyColorFilter(t) {
    for (
      var e = 0.5 * parseInt(t.substr(1, 2), 16),
        a = 0.5 * parseInt(t.substr(3, 2), 16),
        i = 0.5 * parseInt(t.substr(5, 2), 16),
        r = this.imageData,
        s = 0;
      s < r.data.length;
      s += 4
    )
      r.data[s] <= e && (r.data[s] = e),
        r.data[s + 1] <= a && (r.data[s + 1] = a),
        r.data[s + 2] <= i && (r.data[s + 2] = i);
    this.operationEditedCtx.putImageData(r, 0, 0),
      this.operationOrgCtx.putImageData(r, 0, 0),
      this.previewImage();
  }
  colorize(t) {
    var e = 0.5 * parseInt(t.substr(1, 2), 16),
      a = 0.5 * parseInt(t.substr(3, 2), 16),
      i = 0.5 * parseInt(t.substr(5, 2), 16);
    null != this.oldSelectedColorForColorize &&
      ((e = -parseInt(this.oldSelectedColorForColorize.substr(1, 2), 16) + e),
    ...
    for (var r = this.imageData, s = 0; s < r.data.length; s += 4)
      (r.data[s] += e), (r.data[s + 1] += a), (r.data[s + 2] += i);
    this.operationEditedCtx.putImageData(r, 0, 0),
      this.operationOrgCtx.putImageData(r, 0, 0),
      this.previewImage();
  }
  cropSelected() {
    var t = this.previewImageElement.width,
      ...
      h = parseInt(
        parseInt(this.selectRect.style.width.replace(/\D/g, "")) * a
      ),
      o = parseInt(
        parseInt(this.selectRect.style.height.replace(/\D/g, "")) * i
      ),
    ...
    this.operationEditedCtx.clearRect(
      ...
      this.operationOrgCtx.clearRect(
        0,
        0,
        this.operationOrgCtx.width,
        this.operationOrgCtx.height
      ),
